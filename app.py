...
# В DOCX используется цвет шрифта, а в обычном TXT — цветной Unicode-маркер,
# поскольку формат TXT не поддерживает оформление текста.
SPEAKER_VISUAL_STYLES = (
    ("C62828", "🟥"),
    ("1565C0", "🟦"),
    ("2E7D32", "🟩"),
    ("8E24AA", "🟪"),
...
def speaker_visual_style(speaker_name: str) -> tuple[str, str]:
    speaker_id = next(
        (
            identifier
            for identifier, display_name in SPEAKER_NAMES.items()
            if display_name == speaker_name
        ),
        speaker_name,
    )
    match = re.fullmatch(r"SPEAKER_(\d+)", speaker_id)
    if match:
        style_index = int(match.group(1))
    else:
        style_index = sum(ord(character) for character in speaker_id)
    return SPEAKER_VISUAL_STYLES[style_index % len(SPEAKER_VISUAL_STYLES)]


def split_speaker_line(line: str) -> tuple[str, str, str] | None:
    match = SPEAKER_LINE_PATTERN.fullmatch(line)
    if match is None:
        return None
    return (
...
def add_speaker_markers_to_text(text: str) -> str:
    ...
    for line in text.splitlines():
        ...
        _docx_color, marker = speaker_visual_style(speaker_name)
        suffix = f": {content}" if content else ":"
        rendered_lines.append(f"{timestamp}{marker} {speaker_name}{suffix}")
    return "\n".join(rendered_lines)


def word_run_xml(text: str, color: str | None = None, bold: bool = False) -> str:
    if not text:
        return ""
    ...
    run_properties = (
        f"<w:rPr>{''.join(properties)}</w:rPr>" if properties else ""
    )
    return (
...
def word_paragraph_xml(line: str) -> str:
    ...
    runs = [
        word_run_xml(timestamp),
        word_run_xml("■ ", color=docx_color, bold=True),
...
def write_docx(path: Path, text: str) -> None:
    ...
    body = "".join(word_paragraph_xml(line) for line in paragraphs)
...
def write_transcript(path: Path, text: str, output_format: str) -> None:
    ...
    if output_format == "docx":
        ...
        decorated_text = add_speaker_markers_to_text(text)
        path.write_text(
            decorated_text + ("\n" if decorated_text else ""),
            encoding="utf-8-sig",
        )
...
def renumber_speakers_by_first_appearance(
    ...
    for turn in sorted(turns, key=lambda item: (item.start, item.end)):
        if turn.speaker not in speaker_mapping:
            speaker_number = len(speaker_mapping)
            speaker_mapping[turn.speaker] = f"SPEAKER_{speaker_number:02d}"

        normalized_turns.append(
            SpeakerTurn(
                start=turn.start,
                end=turn.end,
                speaker=speaker_mapping[turn.speaker],
            )
        )
...
def diarize_audio(
    ...
    turns = renumber_speakers_by_first_appearance(turns)
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        super().__init__()
        self.setWindowTitle("Система распознавания речи v.1.2")
